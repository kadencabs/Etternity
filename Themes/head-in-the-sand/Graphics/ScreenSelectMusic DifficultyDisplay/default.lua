...
-- 			local Meter = self:GetChild("Meter"
		...
		LoadActor("_barpeice " .. sDifficulty ) .. {
			...
			ShowCommand=function(self)
				self:stoptweening():linear(0.1):diffuse(CustomDifficultyToColor( sDifficulty ))
			end;
			HideCommand=function(self)
				self:stoptweening():decelerate(0.05):diffuse(CustomDifficultyToDarkColor( sDifficulty ))
			end;
		...
		LoadFont("StepsDisplay","Meter") .. {
			...
			InitCommand=function(self)
				self:x(-64-8+tLocation[sDifficulty]):shadowlength(1):zoom(0.75):diffuse(CustomDifficultyToColor( sDifficulty ))
			end;
		};
	};
end
