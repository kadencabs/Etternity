...
local t = Def.ActorFrame {
	LoadActor(THEME:GetPathB("_frame","3x1"),"rounded fill", 192-8) .. {
		OnCommand=function(self)
			self:diffuse(color("#333333")):diffusealpha(0.875)
		end;
	};
	...
	LoadFont("Common Normal") .. {
		...
		InitCommand=function(self)
			self:x(-104):maxwidth(32)
		end,
		OnCommand=function(self)
			self:diffuse(PlayerColor(PlayerNumber)):shadowlength(1)
		end
	},
	LoadFont("Common Normal") .. {
		Text=bpm_text;
		Name="BPMRangeOld",
		InitCommand=function(self)
			self:x(-40):maxwidth(88/bpm_text_zoom)
		end,
		OnCommand=function(self)
			self:shadowlength(1):zoom(bpm_text_zoom)
		end
	},
