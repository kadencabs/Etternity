...
for i=1,#statList do
	...
	as[#as+1] = Def.ActorFrame {
		...
		Def.Quad {
			InitCommand=function(self)
				self:x(128):y(40):horizalign(right):vertalign(bottom)
			end;
			OnCommand=function(self)
				self:zoomto(80,18):diffuse(Color.Black):diffusealpha(0.5):fadeleft(0.5)
			end;
		};
		LoadFont("Common Normal") .. {
			Text=FormatNumberAndSuffix(statList[j]:GetStageIndex()+1);
			InitCommand=function(self)
				self:x(128-4):y(40-4):horizalign(right):vertalign(bottom)
			end;
			OnCommand=function(self)
				self:diffuse(StageToColor(statList[j]:GetStage())):zoom(0.675):shadowlength(1)
			end;
		};
	};
end
