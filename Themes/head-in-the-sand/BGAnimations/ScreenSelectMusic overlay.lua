...
local t = Def.ActorFrame {
	...
	Def.Quad {
		InitCommand=function(self)
			self:zoomto(SCREEN_CENTER_X+80,SCREEN_HEIGHT)
		end;
		OnCommand=function(self)
			self:diffuse(Color.Black):diffusealpha(0.75):fadeleft(32/SCREEN_CENTER_X):faderight(32/SCREEN_CENTER_X)
		end;
	};
};
--
for i=1,#num_players do
	local f = Def.ActorFrame {
		InitCommand=function(self)
			self:x(-128+PositionItem(i,#num_players))
		end;
		UnchosenCommand=function(self)
			self:finishtweening():bounceend(0.25):zoom(1)
		end;
		ChosenCommand=function(self)
			self:stoptweening():bouncebegin(0.3):zoom(0)
		end;
		--
		...
		Def.Quad {
			...
			OnCommand=function(self)
				self:diffuse(PlayerColor(num_players[i])):shadowlength(1):linear(0.25):zoomtowidth(80):fadeleft(0.5):faderight(0.5)
			end;
		};
		LoadFont("Common Bold") .. {
			...
			OnCommand=function(self)
				self:shadowlength(1):diffuse(PlayerColor(num_players[i]))
			end;
		};
		LoadFont("Common Bold") .. {
			Text="PRESS";
			...
			OnCommand=function(self)
				self:shadowlength(1):pulse():effectmagnitude(1,1.125,1):effectperiod(0.5)
			end;
		};
...
t.InitCommand=function(self)
	self:Center():x(SCREEN_CENTER_X*1.5):diffusealpha(0)
end;
