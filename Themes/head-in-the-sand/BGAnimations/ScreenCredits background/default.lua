...
local t = Def.ActorFrame {
	...
	LoadActor("_space")..{
		InitCommand=function(self)
			self:y(-SCREEN_HEIGHT*1.5):fadebottom(0.125):fadetop(0.25)
		end;
		OnCommand=function(self)
			self:linear(scrolltime):addy(SCREEN_HEIGHT*1.5825)
		end;
	};
	Def.Quad {
		InitCommand=function(self)
		...
		OnCommand=function(self)
			self:diffusecolor(color("#FFCB05")):diffusebottomedge(color("#F0BA00")):diffusealpha(0.45)
		end;
	};
	LoadActor("_grid")..{
		InitCommand=function(self)
		...
		OnCommand=function(self)
			self:zoomto(SCREEN_WIDTH+1,SCREEN_HEIGHT):diffuse(Color("Black")):diffuseshift():effecttiming((1/8)*4,0,(7/8)
		end;
