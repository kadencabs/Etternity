...
local number_entry= new_numpad_entry{
	...
	InitCommand= function(self)
		self:diffusealpha(0):xy(_screen.cx, _screen.cy * 1.5)
	end,
	value = LoadFont("Common Large") .. {
		...
		OnCommand=function(self)
			self:zoom(0.75):diffuse(PlayerColor(PLAYER_1)):strokecolor(ColorDarkTone(PlayerColor(PLAYER_1)))
		end;
	...
	cursor = Def.ActorFrame {
		...
		LoadActor( THEME:GetPathG("_frame", "1D"),
			...
			OnCommand=function(self)
				self:diffuse(PlayerDarkColor(PLAYER_1))
			end,
		...
		if item.item_type == "list" then
			...
			value_args[#value_args+1]= LoadActor(THEME:GetPathG("_StepsDisplayListRow","arrow")) .. {
				...
				PressLeftCommand= function(self)
					self:finishtweening():zoom(1.5):smooth(0.25):zoom(1)
				end,
			}
			value_args[#value_args+1]= LoadActor(THEME:GetPathG("_StepsDisplayListRow","arrow")) .. {
				InitCommand= function(self)
