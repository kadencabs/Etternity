...
t[#t+1] = Def.ActorFrame {
	InitCommand=function(self)
		self:x(SCREEN_CENTER_X/3):y(SCREEN_CENTER_Y)
	end;
	LoadFont("Common Normal") .. {
		...
		InitCommand=function(self)
			self:y(-4):shadowlength(1):diffuse(Color("Red")):zoom(0.5)
		end
	};
}
...
for i=1,#histogram do
	...
	t[#t+1] = Def.Quad {
		InitCommand=function(self)
			self:diffuse(Color("Red")):zoomtowidth(300/#histogram-1):zoomtoheight(histogram[#histogram-i]/maxValue*150):x(SCREEN_CENTER_X+100+300/#histogram*i+offset):y(SCREEN_CENTER_Y-histogram[#histogram-i]/maxValue*150/2)
		end;
	}
end
...
t[#t+1] = Def.Quad {
	InitCommand=function(self)
		self:x(SCREEN_CENTER_X+250):y(SCREEN_CENTER_Y+3):zoomtowidth(300):zoomtoheight(5):diffuse(color("#FFFFFF"))
	end;
}

t[#t+1] = Def.Quad {
	InitCommand=function(self)
		self:x(SCREEN_CENTER_X+252):y(SCREEN_CENTER_Y-75):zoomtowidth(1):zoomtoheight(150):diffuse(color("#000000"))
	end;
}
...
t[#t+1] = Def.ActorFrame {
	InitCommand=function(self)
		self:x(SCREEN_CENTER_X+125):y(SCREEN_CENTER_Y+15)
	end;
	LoadFont("Common Normal") .. {
		Text="Early";
		InitCommand=function(self)
			self:shadowlength(1):diffuse(Color("#FF0000")):zoom(0.5)
		end
	};
}
...
t[#t+1] = Def.ActorFrame {
	InitCommand=function(self)
		self:x(SCREEN_CENTER_X+375):y(SCREEN_CENTER_Y+15)
	end;
	LoadFont("Common Normal") .. {
		Text="Late";
		InitCommand=function(self)
