...
local function CreateStats( pnPlayer )
	...
	local aLabel = LoadFont("Common Normal") .. {
		 InitCommand=function(self)
			self:zoom(0.5):shadowlength(1):horizalign(left)
		end
	};
  ...
  for k, v in pairs(tStats) do
	...
	t[#t+1] = Def.ActorFrame {
		...
		LoadActor(THEME:GetPathG("ScreenTitleMenu","PreferenceFrame")) .. {
			InitCommand=function(self)
				self:zoom(0.875):diffuse(PlayerColor( pnPlayer ))
			end;
		};
		
		aLabel .. {
			 Text=THEME:GetString("ScreenEvaluation","ITG DP:"); InitCommand=function(self)
				self:x(-64)
			end
		};
		
		aText .. {
			 Text=string.format("%04i",tValues["ITG"]); InitCommand=function(self)
				self:x(-8):y(5):vertalign(bottom):zoom(0.6)
			end
		};
		
		aText .. {
			 Text="/"; InitCommand=function(self)
				self:x(28):y(5):vertalign(bottom):zoom(0.5):diffusealpha(0.5)
			end
		};
		
		aText .. {
			 Text=string.format("%04i",tValues["ITG_MAX"]); InitCommand=function(self)
				self:x(32):y(5):vertalign(bottom):zoom(0.5)
			end
		};
	};
	t[#t+1] = Def.ActorFrame {
		...
		aLabel .. {
			 Text=THEME:GetString("ScreenEvaluation","MIGS DP:"); InitCommand=function(self)
				self:x(-64)
			end
		};
		
		aText .. {
			 Text=string.format("%04i",tValues["MIGS"]); InitCommand=function(self)
				self:x(-8):y(5):vertalign(bottom):zoom(0.6)
			end
		};
		...
		aText .. {
			 Text=string.format("%04i",tValues["MIGS_MAX"]); InitCommand=function(self)
				self:x(32):y(5):vertalign(bottom):zoom(0.5)
			end
		};
	};
...
t[#t+1] = Def.ActorFrame {
	InitCommand=function(self)
		self:hide_if(not GAMESTATE:IsPlayerEnabled(PLAYER_1)):x(WideScale(math.floor(SCREEN_CENTER_X*0.3)-8,math.floor(SCREEN_CENTER_X*0.5)-8)):y(SCREEN_CENTER_Y-34)
	end;
	CreateStats( PLAYER_1 );
};
t[#t+1] = Def.ActorFrame {
	InitCommand=function(self)
		self:hide_if(not GAMESTATE:IsPlayerEnabled(PLAYER_2)):x(WideScale(math.floor(SCREEN_CENTER_X*1.7)+8,math.floor(SCREEN_CENTER_X*1.5)+8)):y(SCREEN_CENTER_Y-34)
	end;
