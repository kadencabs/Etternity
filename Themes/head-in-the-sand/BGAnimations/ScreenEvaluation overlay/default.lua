...
local function CreateStats( pnPlayer )
	...
	local aLabel = LoadFont("Common Normal") .. {
		 InitCommand=function(self)
			self:zoom(0.5):shadowlength(1):horizalign(0)
		end
	};
	...
	t[#t+1] = Def.ActorFrame {
		...
		aText .. {
			 Text=string.format("%04i",tValues["ITG"]); InitCommand=function(self)
				self:x(-8):y(5):vertalign(1):zoom(0.6)
			end
		};
		
		aText .. {
			 Text="/"; InitCommand=function(self)
				self:x(28):y(5):vertalign(1):zoom(0.5):diffusealpha(0.5)
			end
		};
		
		aText .. {
			 Text=string.format("%04i",tValues["ITG_MAX"]); InitCommand=function(self)
				self:x(32):y(5):vertalign(1):zoom(0.5)
			end
		};
	};
	t[#t+1] = Def.ActorFrame {
...
t[#t+1] = Def.ActorFrame {
	InitCommand=function(self)
		self:x(WideScale(math.floor(SCREEN_CENTER_X*0.3)-8,math.floor(SCREEN_CENTER_X*0.5)-8)):y(SCREEN_CENTER_Y-20)
	end;
