...
local stage_num_actor= THEME:GetPathG("ScreenStageInformation", "Stage 1st", true)
if stage_num_actor ~= "" and FILEMAN:DoesFileExist(stage_num_actor) then
	...
	stage_num_actor= Def.BitmapText{
		Font= "Common Normal",  Text= "1st Stage",
		InitCommand= function(self)
			self:zoom(1.5)
...
t[#t+1] = Def.ActorFrame {
	...
	OnCommand=function(self)
		self:stoptweening():zoom(1.25):decelerate(3):zoom(1)
	end;
	stage_num_actor .. {
		OnCommand=function(self)
			self:diffusealpha(0):linear(0.25):diffusealpha(1):sleep(1.75):linear(0.5):zoomy(0):zoomx(2):diffusealpha(0)
		end;
	};
};
...
t[#t+1] = Def.ActorFrame {
  InitCommand=function(self)
  	self:x(SCREEN_CENTER_X):y(SCREEN_CENTER_Y+96)
  end;
  OnCommand=function(self)
  	self:stoptweening():addy(-16):decelerate(3):addy(16)
  end;
	LoadFont("Common Normal") .. {
		...
		InitCommand=function(self)
			self:strokecolor(Color("Outline")):y(-20)
		end;
		OnCommand=function(self)
			self:diffusealpha(0):linear(0.5):diffusealpha(1):sleep(1.5):linear(0.5):diffusealpha(0)
		end;
	};
	LoadFont("Common Normal") .. {
	...
	LoadFont("Common Normal") .. {
		InitCommand=function(self)
			self:strokecolor(Color("Outline")):diffuse(Color("Orange")):diffusebottomedge(Color("Yellow")):zoom(0.75):y(20)
		end;
		BeginCommand=function(self)
			local text = "";
			local SongOrCourse;
