...
local function StepsDisplay(pn)
	...
	if pn == PLAYER_1 then
		t.CurrentStepsChangedMessageCommand=function(self) set(self, pn); end;
...
local function PercentScore(pn)
	local t = LoadFont("Common normal")..{
		InitCommand=function(self)
			self:zoom(0.625):shadowlength(1)
		end;
		...
		SetCommand=function(self)
			...
			if SongOrCourse and StepsOrTrail then
				...
				local topscore = GetDisplayScore()
				if topscore then
	...
	if pn == PLAYER_1 then
		t.CurrentStepsChangedMessageCommand=function(self)
			self:playcommand("Set")
		end;
...
if not GAMESTATE:IsCourseMode() then
	...
	t[#t+1] = Def.ActorFrame {
		OnCommand=function(self)
			self:draworder(105):x(SCREEN_CENTER_X-256):y(SCREEN_CENTER_Y-84):zoom(0):sleep(0.5):decelerate(0.25):zoom(1):SetUpdateFunction(CDTitleUpdate)
		end;
		OffCommand=function(self)
			self:bouncebegin(0.15):zoomx(0)
		end;
		Def.Sprite {
			Name="CDTitle";
			OnCommand=function(self)
				self:draworder(106):shadowlength(1):zoom(0.75):diffusealpha(1):zoom(0):bounceend(0.35):zoom(0.75):spin():effectperiod(2):effectmagnitude(0,180,0)
			end;
...
if GAMESTATE:IsCourseMode() then
	t[#t+1] = Def.ActorFrame {
		Def.Quad {
			InitCommand = function(self)
				self:x(THEME:GetMetric(Var "LoadingScreen","CourseContentsListX"));
				self:y(THEME:GetMetric(Var "LoadingScreen","CourseContentsListY") - 118);
				self:zoomto(256+32,192);
			end,
			OnCommand=function(self)
				self:diffuse(Color.Green):MaskSource()
			end;
		};
		Def.Quad {
			InitCommand = function(self)
				self:x(THEME:GetMetric(Var "LoadingScreen","CourseContentsListX"));
				self:y(THEME:GetMetric(Var "LoadingScreen","CourseContentsListY") + 186);
				self:zoomto(256+32,64);
			end,
			OnCommand=function(self)
				self:diffuse(Color.Blue):MaskSource()
			end;
		};
	};
