		...
		heart_entries[pn]= new_numpad_entry{
			...
			value = LoadFont("Common Large") .. {
				...
				OnCommand=function(self)
					self:zoom(0.75):diffuse(PlayerColor(pn)):strokecolor(ColorDarkTone(PlayerColor(pn)))
				end;
			...
			button = LoadFont("Common Normal") ..{
				...
				OnCommand=function(self)
					self:diffuse(color("0.8,0.8,0.8,1")):zoom(0.875)
				end,
				GainFocusCommand=function(self)
					self:finishtweening():decelerate(0.125):zoom(1):diffuse(Color.White)
				end,
				LoseFocusCommand=function(self)
					self:finishtweening():smooth(0.1):zoom(0.875):diffuse(color("0.8,0.8,0.8,1"))
				end
			},
			...
			cursor = Def.ActorFrame {
				...
				LoadActor( THEME:GetPathG("_frame", "1D"),
					...
					FitCommand=function(self, param)
						...
						self:playcommand("SetSize",{
							 Width=param:GetWidth()+cursor_width_padding, tween=function(self)
			...
			prompt = LoadFont("Common Bold") .. {
				...
				OnCommand=function(self)
					self:shadowlength(1):skewx(-0.125):diffusebottomedge(color("#DDDDDD")):strokecolor(Color.Outline)
				end;
			},
			max_value= 300,
			auto_done_value= 100,
...
local args= {
	...
	Def.Quad {
		InitCommand=function(self)
			self:xy(SCREEN_CENTER_X+1, SCREEN_CENTER_Y-100+1):zoomto(2,2)
		end;
		OnCommand=function(self)
			self:diffuse(Color.Black):diffusealpha(0.5):linear(0.25):zoomtowidth(420):fadeleft(0.25):faderight(0.25)
		end;
	};
	Def.Quad {
		InitCommand=function(self)
			self:xy(SCREEN_CENTER_X, SCREEN_CENTER_Y-100):zoomto(2,2)
		end;
		OnCommand=function(self)
			self:diffuse(color("#ffd400")):shadowcolor(BoostColor(color("#ffd40077"),0.25)):linear(0.25):zoomtowidth(420):fadeleft(0.25):faderight(0.25)
		end;
	};
	Def.BitmapText {
		...
		InitCommand= function(self)
			self:xy(SCREEN_CENTER_X, SCREEN_CENTER_Y-128):diffuse(Color.White)
		end,
		OnCommand=function(self)
			self:skewx(-0.125):diffuse(color("#ffd400")):strokecolor(ColorDarkTone(color("#ffd400")))
		end}
	,
	...
	Def.BitmapText{
		...
		InitCommand= function(self)
			self:xy(SCREEN_CENTER_X, SCREEN_CENTER_Y+192-32):diffuse(Color.White)
		end,
		OnCommand= function(self)
			self:shadowlength(1)
		end,
	Def.BitmapText{
		...
		InitCommand= function(self)
			self:xy(SCREEN_CENTER_X, SCREEN_CENTER_Y+192-8):diffuse(Color.White)
		end,
		OnCommand= function(self)
...
for pn in ivalues(GAMESTATE:GetEnabledPlayers()) do
	args[#args+1] = LoadActor(THEME:GetPathB("_frame","3x3"),"rounded black", 128, 192) .. {
		InitCommand=function(self)
			self:x(heart_xs[pn]):y(SCREEN_CENTER_Y+28)
		end,
	}
end
