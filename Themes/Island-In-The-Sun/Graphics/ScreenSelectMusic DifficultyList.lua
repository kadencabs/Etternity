...
-- 			self:setaux(1);
	...
	Def.StepsDisplayList {
		...
		CursorP1 = Def.ActorFrame {
			...
			PlayerJoinedMessageCommand=function(self, params)
				if params.Player == PLAYER_1 then
					self:visible(true);
					(function(self) self:zoom(0):bounceend(1):zoom(1) end)(self);
				end;
			end;
			PlayerUnjoinedMessageCommand=function(self, params)
				if params.Player == PLAYER_1 then
					self:visible(true);
					(function(self) self:bouncebegin(1):zoom(0) end)(self);
				end;
			end;
			...
			LoadFont("_roboto condensed Bold 48px") .. {
				Text="P1";
				InitCommand=function(self) self:horizalign(center):x(8):diffuse(ColorDarkTone(PlayerColor(PLAYER_1))) end;
		...
		CursorP2 = Def.ActorFrame {
			...
			LoadActor(THEME:GetPathG("_StepsDisplayListRow","Cursor")) .. {
				InitCommand=function(self) self:diffuse(ColorLightTone(PlayerColor(PLAYER_2))):x(-8):zoom(0.75):zoomx(-0.75) end;
			};
			LoadFont("_roboto condensed Bold 48px") .. {
				Text="P2";
				InitCommand=function(self) self:horizalign(center):x(-8):diffuse(ColorDarkTone(PlayerColor(PLAYER_2))) end;
				OnCommand=function(self) self:zoom(0.5) end;
			};
		};
