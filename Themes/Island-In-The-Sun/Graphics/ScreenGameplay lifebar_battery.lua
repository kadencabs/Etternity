...
t[#t+1] = Def.ActorFrame {
	...
	Def.Quad {
		...
		DeadCommand=function(self) self:stopeffect():diffuse(color("#000000")) end,
	},
	Def.Quad {
		Name="Fill",
		InitCommand=function(self) self:x(-life_meter_width/2):zoomto(life_meter_width,life_meter_height):horizalign(left) end,
		OnCommand=function(self) self:diffuse(PlayerColor(pn)) end,
		--
		HotCommand=function(self) self:glowshift():effectclock('beat') end,
		AliveCommand=function(self) self:stopeffect() end,
		DangerCommand=function(self) self:diffuseshift():effectclock('beat'):effectcolor1(PlayerColor(pn)):effectcolor2(PlayerDarkColor(pn)) end,
		DeadCommand=function(self) self:stopeffect() end
	},
