...
local function ShowProtiming()
  if GAMESTATE.IsDemonstration and GAMESTATE:IsDemonstration() then
    return false
  else
...
t[#t+1] = Def.ActorFrame {
	...
	Def.Quad {
		...
		OnCommand=function(self) self:diffuse(Color("Black")):diffusetopedge(color("0.1,0.1,0.1,1")):diffusealpha(0.8):shadowlength(2) end;
	};
	Def.Quad {
	...
	Def.Quad {
		...
		InitCommand=function(self) self:visible(false):y(32):zoomto(scale(PREFSMAN:GetPreference("TimingWindowSecondsW2"),0,PREFSMAN:GetPreference("TimingWindowSecondsW3"),0,ProtimingWidth-4),16-4) end;
		ResetCommand=function(self) self:finishtweening():diffusealpha(1):visible(false) end;
		OnCommand=function(self) self:diffuse(GameColor.Judgment["JudgmentLine_W2"]) end;
	};
	Def.Quad {
		Name="ProtimingGraphWindowW1";
		InitCommand=function(self) self:visible(false):y(32):zoomto(scale(PREFSMAN:GetPreference("TimingWindowSecondsW1"),0,PREFSMAN:GetPreference("TimingWindowSecondsW3"),0,ProtimingWidth-4),16-4) end;
		ResetCommand=function(self) self:finishtweening():diffusealpha(1):visible(false) end;
...
-- 		c.ProtimingGraphAverage:zoomtowidth( clamp(MakeAverage( tTotalJudgments ) * 1880,0,188) );
		...
		(function(self) self:sleep(2):linear(0.5):diffusealpha(0) end)(c.ProtimingGraphBG);
		(function(self) self:sleep(2):linear(0.5):diffusealpha(0) end)(c.ProtimingGraphUnderlay);
		(function(self) self:sleep(2):linear(0.5):diffusealpha(0) end)(c.ProtimingGraphWindowW3);
		(function(self) self:sleep(2):linear(0.5):diffusealpha(0) end)(c.ProtimingGraphWindowW2);
		(function(self) self:sleep(2):linear(0.5):diffusealpha(0) end)(c.ProtimingGraphWindowW1);
		(function(self) self:sleep(2):linear(0.5):diffusealpha(0) end)(c.ProtimingGraphFill);
		(function(self) self:sleep(2):linear(0.5):diffusealpha(0) end)(c.ProtimingGraphAverage);
		(function(self) self:sleep(2):linear(0.5):diffusealpha(0) end)(c.ProtimingGraphCenter);
	end;
