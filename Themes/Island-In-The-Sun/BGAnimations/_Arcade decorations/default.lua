...
t[#t+1] = Def.ActorFrame {
	...
	LoadActor(THEME:GetPathB("_frame","3x1"),"rounded fill", 250-32) .. {
		OnCommand=function(self) self:diffuse(color("#8C1940")):diffusealpha(1) end;
	};
	LoadFont("Common Italic Condensed") .. {
		InitCommand=function(self) self:zoom(1):shadowlength(1):strokecolor(Color("Outline")):diffuse(color("#FAB56B")):diffusetopedge(color("#F2D5A2")):uppercase(true) end;
		Text="...";
