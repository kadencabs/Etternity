...
struct nps
{
	...
	static void actual_cancer(Calc& calc, const int& hand)
	{
		...
		for (auto itv = 0; itv < calc.numitv; ++itv) {
			...
			for (auto row = 0; row < calc.itv_size.at(itv); ++row) {
				...
				for (size_t col_index = 0; col_index < calc.col_masks.size();
					 ++col_index) {
			...
			for (auto& fing : fingy) {
				if (fing.last_row_time != s_init) {
					auto ms_est =
					  scaly_ms_estimate(fing.ms, scaler_for_ms_base);
					estimates.push_back(ms_est);
				}
			}

			// sort largest first and zero pad to hand note count
			std::sort(estimates.begin(), estimates.end(), [](auto a, auto b) {
				return a > b;
			});
			estimates.resize(column_count(calc.hand_col_masks[hand]));

			// average fingy estimates for msbase. makes sense for 4k where
			// there are 2 estimates. weighted_average fudges the 1/2 factor so
			// generalising the 4k weights to nk is ?? so just telescope
			float msdiff = estimates[0];
			for (size_t i = 1; i < estimates.size(); ++i) {
				msdiff = weighted_average(msdiff,
										  estimates[i],
	...
	static void grindscale(Calc& calc)
	{
		...
		if (populated_intervals > 0) {
			// const auto empty_intervals = static_cast<float>(calc.numitv -
			// populated_intervals);
...
#pragma endregion params and param map
  ...
  private:
						  ...
						  const float& row_time)
	{
		...
		// geometric mean of ms times since (last note in this column) and (last
		// note in the other column)
		...
		auto pineapple =
		  seq._mw_any_ms.get_cv_of_window((unsigned)chaos_comp_window);
		...
		auto sequins = seq._mw_sc_ms[col_right].get_cv_of_window(
		...
		// >0 = uneven trill
		// huge = one column is hitting notes faster than the other (maybe
		// minijack in pattern)
	...
	float get_total_for_windowf(
	  const std::array<int, max_rows_for_single_interval>& arr,
	  const unsigned window) const
	{
		float o = 0.F;
	...
	template<typename T>
	void insert(std::array<T, max_rows_for_single_interval>& arr, const T value)
	{
...
struct oversimplified_jacks
{
	...
	void operator()(const int& column, const float& now)
	{
		for (auto& seq : sequencers) {
			seq.check_reset(now);
		}
