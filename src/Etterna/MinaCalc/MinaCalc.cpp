			   ...
			   const float score_goal) -> std::vector<float>
{
	...
	for (auto cur_iteration = 0; cur_iteration < num_offset_passes;
		...
		float agg = aggregate_skill(
		  iteration_skillet_values, 0.25L, (float)1.11, 0.0, (float)10.24);
...
get_module()
{
	...
	static Php::Extension myExtension("stable_calc",
									  std::to_string(GetCalcVersion()).c_str());
	...
	static Php::Extension myExtension("testing_calc",
									  std::to_string(GetCalcVersion()).c_str());
